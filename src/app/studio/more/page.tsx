...
const MORE_ITEMS: { label: string; icon: IconName; href: string }[] = [
  ...
  { label: 'Zurueck zur App', icon: 'arrow-left', href: '/' },
];
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {MORE_ITEMS.map((item) => (
          <Link key={item.href} href={item.href} className="glass-card rounded-xl p-4 flex items-center gap-3 no-underline transition-transform duration-200 hover:-translate-y-0.5" style={{ background: 'var(--card-bg)' }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--text-h)' }}>{item.label}</span>
