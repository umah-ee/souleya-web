        ...
        <>
          <div className="flex gap-2 mb-6 flex-wrap">
            {courses.map((c) => (
              <button key={c.id} onClick={() => setSelectedCourse(c.id)} className="border-none cursor-pointer" style={{
                padding: '6px 14px', borderRadius: 20, fontSize: 10, letterSpacing: '1px',
          ...
          {enrollments.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {enrollments.map((e) => (
                <div key={e.id} className="glass-card rounded-xl p-4 flex items-center gap-3" style={{ background: 'var(--card-bg)' }}>
