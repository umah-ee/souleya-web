...
import { updateEvent, geocodeLocation } from '@/lib/events';
...
export default function EditEventModal({ event, onClose, onUpdated }: Props) {
  ...
  const [category, setCategory] = useState<'meetup' | 'course'>(event.category);
  const [locationName, setLocationName] = useState(event.location_name);
  const [locationAddress, setLocationAddress] = useState(event.location_address ?? '');
  const [locationLat, setLocationLat] = useState<number | null>(event.location_lat);
  const [locationLng, setLocationLng] = useState<number | null>(event.location_lng);
  const [date, setDate] = useState(extractDate(event.starts_at));
  const [startTime, setStartTime] = useState(extractTime(event.starts_at));
  const [endTime, setEndTime] = useState(event.ends_at ? extractTime(event.ends_at) : '');
    ...
    >
      ...
      >
        ...
        <div className="px-5 pt-4 pb-2 flex items-center justify-between flex-shrink-0">
          <h2 className="font-heading text-lg" style={{ color: 'var(--text-h)' }}>Event bearbeiten</h2>
          <button
            onClick={onClose}
